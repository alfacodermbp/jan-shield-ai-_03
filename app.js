...
async function renderCommandCenter() {
  ...
  document.querySelectorAll('[data-systemic-id]', panels).forEach((item) => { item.onclick = systemicDialog; });
}
...
async function complaintQueue(filters = {}) {
  try { const all = await api('/api/complaints'); const complaints = all.filter((complaint) => (!filters.priority || complaint.priority === filters.priority) && (!filters.active || !['RESOLVED', 'REJECTED'].includes(complaint.status))); const body = complaints.length ? `<div class="app-list">${complaints.map((complaint) => `<button class="app-item" type="button" data-complaint-id="${esc(complaint.id)}"><b>${esc(complaint.id)}</b> · ${esc(complaint.priority)}<br>${esc(complaint.title)}<br><small>${esc(complaint.status)}${complaint.isDemoData ? ' · SYNTHETIC DEMO' : ''}</small></button>`).join('')}</div>` : '<div class="app-empty">No complaints found for this filter.</div>'; const title = filters.priority === 'CRITICAL' ? 'Critical complaints' : filters.active ? 'Active complaints' : 'Complaint queue'; const el = modal(title, body); document.querySelectorAll('[data-complaint-id]', el).forEach((item) => { item.onclick = () => complaintDetail(item.dataset.complaintId); }); } catch (error) { toast(error.message); }
}
